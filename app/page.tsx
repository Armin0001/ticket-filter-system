...
function checkTimestampInRange(timestamp: any, category: string): boolean {
  ...
  switch (category) {
    case "today": return ticketDate >= todayStart && ticketDate < tomorrowStart;
    case "yesterday": return ticketDate >= yesterdayStart && ticketDate < todayStart;
    case "last week": return ticketDate >= lastWeekStart && ticketDate < yesterdayStart;
    case "last month": return ticketDate >= lastMonthStart && ticketDate < lastWeekStart;
    default: return true;
  }
}
...
function getTicketProperty(input: string | null): Record<string, any> {
  ...
  if (priorities.includes(input)) return { priority: [input.charAt(0).toUpperCase() + input.slice(1)] };
...
const PRIORITY_COLOR: Record<string, string> = {
  Urgent: "text-red-500", High: "text-orange-500", Medium: "text-gray-400", Low: "text-green-500",
};
...
function TicketsPage() {
  ...
  const [defaultValueOther, setDefaultValueOther] = useState<Record<string, any>>({});
  const [defaultValueUsers, setDefaultValueUsers] = useState<Record<string, any>>({});
  const [defaultValueOptions, setDefaultValueOptions] = useState<Record<string, any>>({});
  ...
  const urlFilterActive = useMemo(() => !!urlFilter && !("error" in getTicketProperty(urlFilter)), [urlFilter]);
  ...
  const fetchTickets = async () => {
    ...
    try {
      ...
      const activeFilters = Object.keys(myFilters).length !== 0 ? myFilters : mergedObj;
      ...
      if (hasFilters) {
        ...
        Object.entries(activeFilters).forEach(([key, value]) => {
          ...
          if (key === "dateCreated" || key === "lastActivity" || key === "lastMessage") {
          ...
          if (key === "followers" || key === "Tags") {
            myQuery = query(myQuery, where(newKey, "array-contains-any", value));
          } else {
        ...
        let docs = filteredSnapshot.docs.map(d => ({ ...d.data(), id: d.id } as Ticket));
        ...
        if (Object.keys(clientDateFilters).length > 0) {
          docs = docs.filter(ticket => {
            return Object.entries(clientDateFilters).every(([field, category]) => {
        ...
        const list = querySnapshot.docs.map((d) => {
          ...
          return {
            ...
            agent: t.agent, status: t.status, priority: t.priority,
            lastMessage: t.lastMessage, madeBy: t.madeBy,
            attachmentLink: t.attachmentLink, dateCreated: t.dateCreated,
            lastActivity: t.lastActivity, followers: t.followers,
      ...
      if (String(err).includes("YOUR_") || String(err).includes("projectId") || err?.code === "permission-denied") {
  ...
  const logData = () => {
    ...
    const assignedAgents = tickets.filter((item) => item.agent !== "").map(item => item.agent);
    const unassignedAgents = tickets.filter((item) => item.agent === "").map(item => item.agent);
    const complexAttachments = tickets.filter((item) => item.attachmentLink === "true").map(item => item.attachmentLink);
    const emptyAttachments = tickets.filter((item) => item.attachmentLink !== "true").map(item => item.attachmentLink);

    if (obj3["Attachment"]?.[0] === "has") obj3["Attachment"] = complexAttachments;
    if (obj3["Attachment"]?.[0] === "has_not") obj3["Attachment"] = emptyAttachments;
    if (obj3["Assignment"]?.[0] === "assigned") obj3["Assignment"] = assignedAgents;
    if (obj3["Assignment"]?.[0] === "unassigned") obj3["Assignment"] = unassignedAgents;

    if (Object.prototype.hasOwnProperty.call(obj3, "Assignment")) { obj3["agent"] = obj3["Assignment"]; delete obj3["Assignment"]; }
    if (Object.prototype.hasOwnProperty.call(obj3, "Attachment")) { obj3["attachmentLink"] = obj3["Attachment"]; delete obj3["Attachment"]; }
    ...
    if (Object.prototype.hasOwnProperty.call(obj3, "Priority")) {
      obj3["Priority"] = obj3["Priority"].map((item: string) => item.charAt(0).toUpperCase() + item.slice(1));
    }

    if (Object.prototype.hasOwnProperty.call(obj3, "Status")) {
      obj3["Status"] = obj3["Status"].map((item: string) => item.charAt(0).toLowerCase() + item.slice(1));
    }

    if (Object.prototype.hasOwnProperty.call(obj3, "Tag")) {
      obj3["Tags"] = obj3["Tag"].map((item: string) => item.charAt(0).toLowerCase() + item.slice(1));
      delete obj3["Tag"];
    }
  ...
  const handleInputChange = (value: string) => {
    setInputValue(value);
    if (value !== "") {
      const filterBySearch = (list: Ticket[]) => list.filter((item) => item.subject.toLowerCase().includes(value.toLowerCase()));
  ...
  const updateTicket = async (ticketId: string, updatedFields: Partial<Ticket>) => {
    try {
  ...
  const visibleTickets = checkUsed || urlFilterActive ? filteredTickets : tickets;
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8">
        <div className="max-w-lg w-full bg-white rounded-2xl shadow-lg p-8 border border-red-100">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-bold text-xl">!</div>
            <h2 className="text-xl font-semibold text-gray-800">Firebase Configuration Error</h2>
          </div>
          <p className="text-gray-600 text-sm mb-5 leading-relaxed">
            Please check your configuration files under <code className="bg-gray-100 px-1.5 py-0.5 rounded text-red-600 font-mono text-xs">config/firebase.ts</code>.
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <rect x="1" y="1" width="6" height="6" rx="1" fill="white" opacity="0.8"/>
            ...
            {["open","pending","solved","urgent","spam"].map(f => (
              <button
                ...
                className={`px-2.5 py-1 rounded-full capitalize transition-colors ${
                  urlFilter === f ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
                }`}
              >
                {f}
              </button>
      ...
      <main className="max-w-7xl mx-auto px-6 py-6 space-y-4">
        {urlFilterActive && (
          <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-2.5 text-sm">
            ...
            <span className="bg-indigo-600 text-white px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize">{urlFilter}</span>
            <button onClick={() => { setCheckUsed(false); router.push("/"); }} className="ml-auto text-xs text-indigo-500 hover:text-indigo-700 underline">Clear</button>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="grid grid-cols-[2fr_2.5fr_1.5fr_1fr_1fr_1.8fr_40px] px-5 py-3 border-b border-gray-100 text-[11px] font-semibold text-gray-400 uppercase tracking-wider">
            <div>Requester</div><div>Subject</div><div>Agent</div>
          ...
          {loading ? (
            ...
            visibleTickets.map((ticket, index) => (
              <TicketRow key={ticket.id} ticket={ticket} index={index} onUpdate={updateTicket}/>
            ))
          ) : checkUsed ? (
            <div className="flex flex-col items-center justify-center py-24 text-gray-400">
              <div className="text-5xl mb-4">🔍</div>
              <p className="text-sm font-medium text-gray-500">No data on record!</p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-24 text-gray-400">
              <div className="text-5xl mb-4">🎫</div>
              <p className="text-sm font-medium text-gray-500">Create a ticket to get started!</p>
            </div>
          )}
        </div>
      </main>
...
function TicketRow({ ticket, index, onUpdate }: {
  ticket: Ticket;
  index: number;
  ...
  const initials = useMemo(() => {
    return ticket.name ? ticket.name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase() : "?";
  }, [ticket.name]);
  
  const colors = ["bg-indigo-500","bg-violet-500","bg-pink-500","bg-teal-500","bg-orange-500","bg-cyan-500"];
  ...
  return (
    <div className="grid grid-cols-[2fr_2.5fr_1.5fr_1fr_1fr_1.8fr_40px] px-5 py-3.5 border-b border-gray-50 last:border-0 hover:bg-slate-50 transition-colors group items-center">
      <div className="flex items-center gap-2.5 min-w-0">
        <div className={`w-8 h-8 rounded-full ${avatarColor} text-white flex items-center justify-center text-xs font-bold flex-shrink-0`}>{initials}</div>
        <div className="min-w-0">
          <div className="text-sm font-medium text-gray-800 truncate">{getDisplayValue(ticket.name)}</div>
          <div className="text-xs text-gray-400 truncate">{getDisplayValue(ticket.email)}</div>
        </div>
      </div>

      <div className="min-w-0 pr-4">
        <div className="text-sm text-gray-700 truncate">{getDisplayValue(ticket.subject)}</div>
        {ticket.folder && <div className="text-xs text-gray-400 mt-0.5">{ticket.folder}</div>}
      </div>

      <div className="text-sm min-w-0">
        {ticket.agent
          ? <span className="flex items-center gap-1.5 text-gray-600"><span className="w-1.5 h-1.5 bg-green-400 rounded-full flex-shrink-0"/><span className="truncate">{ticket.agent}</span></span>
          : <span className="text-gray-300 italic text-xs">unassigned</span>
        }
      </div>

      <div className="flex justify-center">
        <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${STATUS_COLOR[ticket.status] ?? "bg-gray-100 text-gray-600"}`}>
          {getDisplayValue(ticket.status)}
        </span>
      </div>

      <div className="flex justify-center">
        <span className={`flex items-center gap-1 text-xs font-semibold ${PRIORITY_COLOR[ticket.priority] ?? "text-gray-400"}`}>
          <span className="text-sm leading-none">{PRIORITY_ICON[ticket.priority] ?? "●"}</span>
          {ticket.priority.charAt(0).toUpperCase() + ticket.priority.slice(1)}
        </span>
      </div>

      <div className="text-center text-xs text-gray-400">
        {ticket.lastMessage ? formatTimestampWithTime(ticket.lastMessage) : "unassigned"}
      </div>
        ...
        {menuOpen && (
          <>
            ...
            <div className="absolute right-0 top-full mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 py-1 w-44 text-sm max-h-64 overflow-y-auto">
              <div className="px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider border-b border-gray-100">Change Priority</div>
              {[
                ...
                <button key={p} onClick={() => { onUpdate(ticket.id, { priority: p }); setMenuOpen(false); }}
                  className={`w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center gap-2 ${color}`}>
                  <span>{PRIORITY_ICON[p]}</span> {p}
                </button>
              ))}
              <div className="px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider border-y border-gray-100">Change Status</div>
              {["open","pending","on-hold","solved","closed"].map((s) => (
                <button key={s} onClick={() => { onUpdate(ticket.id, { status: s }); setMenuOpen(false); }}
                  className="w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-50 capitalize">{s}</button>
              ))}
            </div>
          </>
        )}
